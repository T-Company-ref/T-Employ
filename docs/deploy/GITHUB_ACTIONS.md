...
|---------------|------|------------|------------|
| `poll-applicants.yml` | 지원자 목록 경량 폴링 (HTTP+HTML) | 없음 | 평일 08:17~19:17 매시 |
...
| `session-refresh.yml` | 로그인 세션 재발급 | Chromium | 매월 1일 06:30 + 수동 |
...
| 작업 | KST | UTC cron |
|------|-----|----------|
| session-refresh (매월 1일) | 06:30 | `30 21 * * *` + gate(KST day=01) |
...
| poll-applicants | 평일 09:17~19:17 | `17 0-10 * * 1-5` |
...
- 세션 정기 갱신은 **월 1회** (매일 cron gate로 본 작업 스킵)
