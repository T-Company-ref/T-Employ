...
발신 표시: **`T-Employ <tbell.wr@gmail.com>`** → 수신: `yj.kim@tbell.co.kr` (기본 포함)

| 키 | 설명 |
|----|------|
| `GMAIL_USER` | `tbell.wr@gmail.com` |
| `GMAIL_APP_PASSWORD` | Google **앱 비밀번호** 16자리 (일반 비밀번호 불가) |
| `MAIL_FROM` | `T-Employ <tbell.wr@gmail.com>` |

GitHub Secrets: `GMAIL_USER`, `GMAIL_APP_PASSWORD`, `MAIL_FROM` (T-Client 레포와 **키 이름만 동일**, 보낸사람은 T-Employ)
...
`tbell.co.kr` 도메인을 Resend에 인증한 뒤 `RESEND_FROM=T-Employ <noreply@tbell.co.kr>` 사용. `@gmail.com` 발신은 Resend 불가.
