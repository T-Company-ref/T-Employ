...
> **역할**: Phase 0에서 DB/RLS **인프라** 선반영용. 웹 UI(프론트·Auth)는 Phase 3.5에서 별도 진행.

> ✅ 확정된 접속 정보 (2026-07-08 검증 완료)
> - Project ref: `koxsezeotvylkeqeixnb` / Region: `ap-south-1` (Mumbai)
> - `SUPABASE_URL`: `https://koxsezeotvylkeqeixnb.supabase.co`
> - **DATABASE_URL (동작 확인된 pooler)**: `postgresql://postgres.koxsezeotvylkeqeixnb:[PASSWORD]@aws-1-ap-south-1.pooler.supabase.com:5432/postgres`
> - 직접 연결(`db.<ref>.supabase.co`)은 IPv6 전용이라 로컬/GitHub Actions(IPv4)에서 사용 불가 → **반드시 위 Session pooler 사용**
...
이후 프론트엔드(로그인/태그/면접/삭제상태)는 **Phase 3.5** 로 구현됨 → `docs/develop/web-ui.md`.
